...
void TableComponent::parseStyles(const nlohmann::json& properties) {
    ...
    // Parse colors from styles (may be already-resolved values or Token references)
    if (styles.find("header-bg-color") != styles.end()) {
        if (styles["header-bg-color"].is_string()) {
